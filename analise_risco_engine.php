<?php
// ============================================================
// analise_risco_engine.php
// Motor de Subscrição de Risco — SAD Super Seguro
//
...
// Fonte: Tábua BR-EMS 2021 (simplificada)
...
// Fator IMC — relevante para vida e saúde
...
function fatorComprometimento(float $salario, float $valorApli): array {
    ...
    if ($valorApli <= 0) return ['fator'=>1.0, 'obs'=>'Valor de cobertura ainda não informado', 'ratio'=>0];
...
// Fator geográfico por estado (sinistralidade histórica Brasil)
...
function calcularRisco(array $cliente, string $produto, array $extras = []): array {
    ...
    // Valor a ser segurado: informado na própria tela de Análise de Risco
    // (não vem mais do cadastro do usuário — esse campo foi removido de lá)
    $valor  = (float) ($extras['valor_cobertura'] ?? 0);
    ...
    $profAlto = ['policial','bombeiro','minerador','piloto','segurança privada','mergulhador','motociclista profissional'];
    $profMedio= ['médico','enfermeiro','construção civil','eletricista','caminhoneiro'];
...
try {
        ...
        try {
            $stmt = $pdo->query("
                SELECT u.id, u.nome, u.cpf, u.cidade, u.estado,
                       u.datanascimento, u.genero, u.salario,
                       ROUND(COALESCE((
            ...
            // Fallback: coluna 'origem' ainda não existe (migração pendente)
            // Execute migrate_remove_valorapli.sql para corrigir definitivamente.
            if (str_contains($e->getMessage(), 'origem')) {
                $stmt = $pdo->query("
                    SELECT u.id, u.nome, u.cpf, u.cidade, u.estado,
                           u.datanascimento, u.genero, u.salario,
                           ROUND(COALESCE(AVG(c.Response)*10,0),1) AS ultimo_score,
                           COUNT(a.id) AS total_apolices
                    FROM   usuarios u
                ...
                echo json_encode([
                    ...
                    'aviso'    => 'Migração pendente: execute migrate_remove_valorapli.sql para scores precisos.',
                ]);
            } else {
                throw $e;
            }
    ...
    if ($method === 'GET' && isset($_GET['cliente_id'])) {
        ...
        $extras  = [
            ...
            'valor_cobertura' => (float) ($_GET['valor_cobertura'] ?? 0),
        ];
        ...
        // Mapeia produto da URL ('automovel') para o ENUM real do banco ('Auto')
        $mapaTipoSeguro = [
            'vida'        => 'Vida',
            'saude'       => 'Saude',
            'automovel'   => 'Auto',
        ...
        $tipoSeguroDb = $mapaTipoSeguro[$produto] ?? ucfirst($produto);

        // Se nenhum valor foi informado, sugere o valor de cobertura
        // da apólice ativa mais recente do mesmo produto (se existir)
        if ($extras['valor_cobertura'] <= 0) {
            try {
                $stmtAp = $pdo->prepare("
                    SELECT valor_cobertura FROM apolices
                    WHERE usuario_id = :uid AND tipo_seguro = :tipo AND status = 'ativa'
                    ORDER BY id DESC LIMIT 1
                ");
                $stmtAp->execute([':uid' => $id, ':tipo' => $tipoSeguroDb]);
                $apExistente = $stmtAp->fetch();
                if ($apExistente) {
                    $extras['valor_cobertura'] = (float)$apExistente['valor_cobertura'];
                }
            } catch (PDOException $e) {
                // Não deixa a sugestão de valor derrubar a análise inteira
                error_log('Aviso ao sugerir valor_cobertura: ' . $e->getMessage());
            }
        }
        ...
        $resultado['valor_cobertura_usado'] = $extras['valor_cobertura'];
    ...
    if ($method === 'POST') {
        ...
        // Salva na tabela clientes com origem explícita 'analise_risco'
        // (não depende mais de BMI=0 como heurística — usa coluna dedicada)
        $stmt = $pdo->prepare("
            INSERT INTO clientes (usuario_id, Ins_Age, BMI, Response, origem)
            SELECT :uid,
